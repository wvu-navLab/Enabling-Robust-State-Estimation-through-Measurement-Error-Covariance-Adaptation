...
int main(int argc, char** argv) {
        ...
        while (keepGoing) {
                ...
                // if (robustIter != 0){params.maxIterations = 50;}
                optimized_values = GaussNewtonOptimizer(graph, optimized_values).optimize();
                ++numIters;
                ...
                for (int i = 0; i<gnssFactors.size(); i++) {
                        ...
                        cout << graph.at(get<1>(gnssFactors[i]))->residual(optimized_values).transpose() << endl;
                }
