...
int main(int argc, char** argv) {
        ...
        typedef std::tuple<int, int, int, double>  tup;
        ...
        // Read GNSS data
        try {data = readGNSS_SingleFreqTmp(gnssFile); }
        ...
        for(unsigned int i = startEpoch; i < data.size()-1; i++ ) {
                ...
                gnssFactors.push_back( tup(i,++factorCount,bias_counter[svn], elevation));
        ...
        do {
                ...
                residuals.setZero(gnssFactors.size(),3);
                for (int i = 0; i<gnssFactors.size(); i++) {
                        residuals.block(i,0,1,3) << graph.at(get<1>(gnssFactors[i]))->residual(optimized_values).transpose(), get<3>(gnssFactors[i]);

                        res_os << graph.at(get<1>(gnssFactors[i]))->residual(optimized_values).transpose() << " " <<  get<3>(gnssFactors[i]) << endl;
                }
                res_os.close();
                ...
                // Tw_inv = Eigen::MatrixXd::Identity(3,3);
                // means = Eigen::RowVectorXd(3);
                ...
                // Update previous graph with new measuremnt model
